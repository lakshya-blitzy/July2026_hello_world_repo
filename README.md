...
`npm test` or `npm start`. `overrides` holds exactly one entry — `brace-expansion` pinned to `5.0.8`,
which closes advisory GHSA-mh99-v99m-4gvg and takes both audits to zero findings. Keep every
`jest.config.js` pattern brace-free: the `minimatch`/`glob` generation Jest 30.4.2 resolves still
imports the callable default export that `brace-expansion@5` removed, so a brace-bearing pattern
would throw. Neither shipped pattern contains one.
...
0.70 s of runner time (1.21 s under `test:ci`); 13 / 31 / 17 tests in the unit, integration and e2e
subsets; **332 installed packages from 361 lockfile entries** (`lockfileVersion 3`); zero audit
findings on the whole tree and on the production tree, which `npm ls --omit=dev --all` reports as
empty; 100% on all four coverage metrics (9/9, 0/0, 2/2, 9/9); the 14-byte body with digest
`c98c24b6…ad31`; and the 41-byte readiness line. The installed `jest` package is 30.4.2 while its CLI
self-reports 30.4.1, because the constituent packages version independently.
...
event-driven exchange settling (`ST-7`, twice). Three further titles carry both kinds at once, and
the lifecycle tier additionally prefixes its scenario ID (`S1`…`S7`).
